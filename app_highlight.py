...
EVALUATION_METHOD_INFO = """
        ...
        1. 평가를 시작하기 전에, 평가 및 점수 부여 기준을 읽어주세요.
        ...
        6. 각 Context 를 클릭하면, 해당 외부 지식의 내용을 확인하여 어느 부분에서 참조했는지 답변과 비교할 수 있습니다.

        7. 평가 및 점수 부여 기준에 근거하여, 페이지 최하단의 점수를 선택해 주세요.
"""
...
**[점수 부여 기준]**
- **0점** : 무응답. 질문과 전혀 상관없는 답변을 하거나 답변 자체를 거부함.
- **1점** : 환각 발생. 존재하지 않는 Quantum ESPRESSO 플래그를 제시하거나 물리적으로 완전히 잘못된 정보를 제공함.
- **2점** : 미흡. 기술적 오류가 일부 포함됨. 그럴듯해 보이지만 검증되지 않은 정보가 섞여 있어 그대로 믿기 위험함.
- **3점** : 보통. 원론적인 수준의 답변이거나 사용자 의도를 관통하는 디테일 (Nuance) 이 부족함.
- **4점** : 우수함. 기술적 오류가 없으며 실무에 유용함. 다만 표현이 다소 장황하거나 스타일 면에서 약간의 아쉬움이 있음.
- **5점** : 완벽함. 부족한 부분이 없는 완벽한 기술적 답변. Quantum ESPRESSO 사용자에게 즉시 적용 가능한 최적의 정보 제공.
"""
